...
- Installation of the QGIS-Tim and iMOD plugin is part of this Tutorial and the necessary ZIP files are included in the tutorial material. 
- Internet connections is optional for the Tutorial and only required for installation of additional plugins and the use of an online topographic background map.  
...
> a. Check the box to allow *Override System Locale*. 
...
> - Use the browse button (![](figures/tutorial/button-Qgis-browse.png){width=4%}) and from the tutorial material select “…\\QGIS-Tim_training\\dbase\\TopographicMapRijssenhout.png”.
...
(@) Click on the PDOK plugin button (![](figures/tutorial/button-PDOK.png){width=4%}) to open the "PDOK Services Plugin" window. 
...
(@) Use the browse button (![](figures/tutorial/button-Qgis-browse.png){width=4%}) and from the tutorial material select “…\\QGIS-Tim_training\\dbase\\bombs.shp”.
(@) Click on *Add* and *Close* the window.
...
> **Intermezzo:** *Mesh and Raster explained*
> 
> - Raster: is made up of pixels (also referred to as grid cells). They are usually regularly spaced and square but they don’t have to be. Preferred format to create cross sections or evaluate values at your mouse location. 
> - Mesh: an unstructured grid usually with temporal and other components. Preferred format to animate temporal data and to perform calculations.  
...
A single aquifer is of course not enough to describe the geology of our project area. A simplification of the geology is given in the cross section in @fig-photo-fieldwork-liftbomb-crosssection. In this low lying polder, there is a Holocene toplayer of 12 m thickness on top of an aquifer. The surface level of the toplayer is 5 m below see level, it has a resistance of 1500 days and the water level within this polder is 1 meter below surface level. The aquifer has a horizontal permeability of 30 m/d and a thickness of 50 m. The anisotropy factor is 3 resulting in a vertical resistance of 0.1 day per meter. Tim can handle multiple layers so the next step is to bring these parameters into the model.
...
(@) Start editting with the *Toggle Editing Mode*.
...
(@) Start editting with the *Toggle Editing Mode* and use *Add Point Feature* button (![](figures/tutorial/button_AddPointFeature.png){width=4%}) to add 5 points in line with 1 really close to the dewatering well (see @fig-figure_ObservationsLocations).
...
By selecting the "transient" option, columns containing extra parameters necessary for a transient model have been unhided now. First of all we see it in the layer "timml Aquifer:Aquifer" where columns "storage" and "porosity are visible now. 
...
- Detailed: a well can be switched on and off multiple times within the modeled period. Parameters are set in the transient part of the model, so in "ttim Well:DewateringWell".
