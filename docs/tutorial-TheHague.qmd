...
![Schematization of the building area in The Hague city centre (orange: building pit)](figures/tutorial/photo-TheHagueCityCentre.png){width=60% #fig-TheHagueCityCentre fig-align="left"} 
...
![Schematization of the pumping wells, sheet piles and observation locations](figures/tutorial/figure_SchematizationTheHague.png){width=60% #fig-SchematizationTheHague} 
...
![Extra points for segmentation of the sheet pile element](figures/tutorial/figure_SheetPileExtraPoints.png ){width=80% #fig-figure_SheetPileExtraPoints} 
...
![Updated results of groundwater head lowering contours due to 8 wells 50 m3/d and a sheet pile wall with 100 d wall resistance.](figures/tutorial/figure_DrawdownConstructionPit_v01.png){width=80% #fig-DrawdownConstructionPit_v01  fig-align="left"} 
...
![Python script, first part.](figures/tutorial/figure_PythonExport01.png){width=55% #fig-figure_PythonExport01}
...
![Python script, last part.](figures/tutorial/figure_PythonExport02.png){width=55% #fig-figure_PythonExport01}
...
![Result of test run in PTK to check model outcome.](figures/tutorial/figure-PTK03.png){width=40% #fig-figure-PTK03} 
...
(@) Go to the tab *Model* and in the panel *Model code* remove the line for pb0 and the line for pb1. 
...
(@) Then we check the variable distributions again. Next, we open the tab *Calculation*. 
...
(@) Fill in the table according to @fig-Panel-TIMMAquiferAttrTable_v01.

![Table for *ttim Temporal Settings:Aquifer* ](figures/tutorial/Panel-TIMM AquiferAttrTable_v01.png){width=90% #fig-Panel-TIMMAquiferAttrTable_v01}
...
![Table for *ttim Computation Times:Domain* ](figures/tutorial/Panel-TIMM_DomainAttrTable_v01.png){width=40% #fig-Panel-TIMM_DomainAttrTable_v01}
...
![Table for *ttim Observation:observations* ](figures/tutorial/Panel-TIMM_ObservationsAttrTable_v01.png){width=40% #fig-Panel-TIMM_ObservationsAttrTable_v01}

Don’t choose exactly the same time as when changing the flow. So if 60 is an end value, choose 59.9.

In the *Attribute Table* of layer *ttim Well:pumping_wells* we can set the discharge amount. The value might change during time when excavation is considered or construction is performed in phases. For each change we have to add a line of input. <br> We just leave this option without any change.

Another way of inserting stress periods for wells is by going to the *timml Well:pumping_wells* layer. <br> But fill in a general value of 0 m3/d for TimML attributes of the well element when you want to use changing flow in TTIM. Remember: Solution of TTIM is superposed on solution of TIMml.