...
![Schematization of the building area in The Hague city center (orange: building pit)](figures/tutorial/photo-TheHagueCityCenter.png){width=80% #fig-TheHagueCityCenter fig-align="left"} 
...
(@) If you do have an internet connection go to the *Browser* panel on the left open the group *XYZ Tiles*.
(@) Double click on ![](figures/tutorial/button_OpenStreetMap.png){width=15%} to load this map to the panel *Layers*.  
(@) You might close the panel *Browser* now (click on X) in order to give the panel *Layers* the full vertical space. 
...
(@) Click the button *Select by Polygon* and draw a large polygon (> 250x250m) to select the area around the building location between *Parkstraat* and *Oranjestraat* (see @fig-TheHagueCityCenter). Left-click several times and close the polygon with a right click.  
...
> ![](figures/tutorial/figure_ExtractionError.png){width=80% #fig-figure_ExtractionError} <br> QGIS displays this error message in case your polygon is to small. In that case redraw your polygon for a larger area. 
...
(@) From a file manager on you laptop (e.g. Explorer) open the CVS files. 

An example of values from the first CSV are presented in Table 1. In each LHM layer the Aquitard information is positioned on top of Aquifer. 
...
: The characteristics of the subsoil based on LHM data (example, your values may differ)
...
Aquitard resistance in the city is quite high, so a value of 750 days is best guess from experience. <br><br>
...
Closing this table window with the ![](figures/tutorial/Button_CloseWindow.png){width=2.5%} wil not save the filled in data or stop the editing mode! 
...
<r> ?? Hans, waarom moeten de gebruikers hieronder eerst een shape maken? Kunnen we niet beter de shape aanleveren als tutorial achtergrond? 

<r> To help maneuvering in the QGIS we first draw the circumference of the building location in QGIS. We can do it by creating a drawing in the menu option Layer, choosing Create layer, New shapefile layer. We create a new polygon geometry. Don’t forget to set the proper path to your directory and to fill in the right CRS (see @fig-NewShapefileLayer).  

(@) <r> Click in the Layers panel on the shape
(@) <r> and draw with (![](figures/tutorial/button-Qgis-AttributeTable-StartEditingMode.png ){width=4%}) a polygon. 

<r> By using the vertex option you can adjust the geometry shape with help of the measuring tool of QGIS. 
...
(@) Fill in the name of the layer in the popup panel, e.g. 'sheet_pile', and click *OK* and this new Layer is added to the Layers panel.  
...
(@) Draw the sheet pile arround the building pit (start south corner) with your left mouse button and close the feature on the first point. 
(@) In the popup panel fill in resistance (1000 \*) and layer number (layer=0 \**) as in @fig-LeakylinedoubletFeatureAttributes and click OK to close the window.
...
\* Because of a known error in the TimML software, until this issue is resolved, the value of the resistance must be multiplied by the permeability. For normal sheet pile walls a resistance value of 100 d for interlock leakage is a good choice. In this case the chosen value needs to be increased by multiplying with the applicable aquifer permeability in the effected layer (R=100 d becomes R=10*100 d). <br>
...
(@) In GIS-Tim goto the tab *Elements* and select the *Well* element. 
(@) A name for the element can be given in the popup panel, e.g. *pumping_wells*. 
...
(@) Start the editting mode and fill in the values shown in @fig-WellAttributeTable. Dont forget the column 'Label'. <br> The discharge value in the table is a first guess to be adjusted later to desired level of -3.5 m groundwater lowering in the building pit.
(@) Stop the editting mode, save your work and close the window. 
...
(@) Activate the Vertex Tool (![](figures/tutorial/button_VertexTool.png){width=4%}) and then hoover along the sheet pile line in your drawing. The line element near you pointer lights up.
...
(@) Zoom in or out to desired domain for which you want to see the modelresults.
(@) In the QGIS-Tim panel select the tab *Compute* (see @fig-ComputeSettings). 
(@) Select the button "Set to current extent" to define the Domain. 
(@) Grid spacing will follow automatically but for now make the results mesh more dense by changing "Grid spacing" to 5.00 m. 
...
(@) Press the "Compute" button to have the program perform the calculations. A black Python.exe window might pop up (but calculations take place in the background). While the calculation is running the progress is shown in the QGIS button in the task bar.

![Compute settings](figures/tutorial/figure_ComputeSettings.png){width=60% #fig-ComputeSettings fig-align="left"} 

After the calculation you see that the result is automatically added to a new geopackage, probably called “case-TheHague output”. Results are saved both as mesh and raster. Contours are saved as vector. For each format a separate sub group is created. Although these layers / groups are checked, the data is not visible. That is because the geopackage was added last, and QGIS adds layers at the end of the list. <br> Let’s move the layer “OpenStreetMap” to the background.

(@) Select the layer "OpenStreetMap" and drag it with your left mouse button to the bottom of the list of layers.
(@) Uncheck the mes and raster ouput in order to only dispay the isolines on the map.

The lowering in the building pit is shown to be around -7.5 m (<r> ?? Ik krijg 14 m </r> ). To get it around -3.5 m as demanded the well discharge should be decreased accordingly. An extra calculation with half the flow per well will be sufficient. 
...
(@) Uncheck mesh and raster to only visualize contours on the base map as in @fig-DrawdownConstructionPit_v01.
(@) Uncheck contour lines of layer 1 and 2 to get only the result for layer 0 on screen.
...
(@) After changing the value in attribute tables of wall elements, we can compute again, switching off and on the element for the deep wall section in the geopackage (go to tab ?? in the QGIS-Tim panel). 
...
(@) Than we copy the Python file we converted from QGIS-Tim into the Source code window.
