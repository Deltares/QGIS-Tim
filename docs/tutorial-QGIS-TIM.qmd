...
East of the projectare there is a large Lake. What will be the influence of this lake? Let's find out. <br>
In TIM a lake is added as an inhomogeneity within the total model domain. This domain is described in tabel "timml Aquifer:Aquifer" we filled in with the first model. 
...
(@) Fill the Lake feature with the following parameters: aquitard_c = 300, semiconf_top = -10, semiconf_head=-5.
(@) Save your changes and rerun the model. 
(@) Analyse the results and perhaps create contour lines from the tab "Compute". 

![Contour lines after adding the Lake element in the east](figures/tutorial/figure_Contourlines02.png){width=70% #fig-figure_Contourlines02}


You can not miss the effect of the Lake in this calculation. In many cases you like to analyse or even save this difference. Let's see how we can use TIM and QGIS to isolate the effect.     
