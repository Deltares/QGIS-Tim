...
In TIM a lake is added as an inhomogeneity within the total model domain. This domain is described in tabel "timml Aquifer:Aquifer" we filled in with the first model. So we must make a copy of it an change the top.  

(@) On the tab *Elements* in QGIS-Tim click on the element "Polygon Inhomogeneity".
(@) Give the layer a name, e.g. "Lake" and find out that layer "timml Polygon Inhomogeneity:lake" is added to the list.
...
(@) fill in the tabel "timml Polygon Inhomogeneity Properties:lake".
(@) Save your changes and rerun the model. 
