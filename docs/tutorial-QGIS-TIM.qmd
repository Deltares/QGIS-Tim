...
**NB!** All the editing of the parameters and elements we do in your geopackage in the *Layers* panel on the left!   
...
(@) From the menu select *Attribute Tabel* and the table opens in a new window. 
...
See that layer "timml Well:DewateringWell" is added to the timml sub-group in your geopackage with 'point' as geometry. Also in the ttim sub-group an element with the same name is added. This is a tabel that can store transient well data while refering to the x/y locations in the timml layer. Next step is to add the actual wel, both the location and its parameters.  
...
(@) In your geopackage select the layer "timml Aquifer:Aquifer" and click your right mouse button.
(@) From the menu select *Attribute Tabel* and the table opens in a new window. <br> **NB** Alternative is to press Shift + F6 or use the button *Open Attrribute Tabel* (![](figures/tutorial/button-Qgis-OpenAttributeTabel.png){width=4%}).
(@) Our existing single aquifer seems to be gone... That is not the case but a nasty habit of QGIS. To show the aquifer, click on the button ![](figures/tutorial/button_ShowSelectedFeatures.png){width=20%} on the bottom of the table and select *Show All Features*.
(@) Start the editing mode with a click on the *Toggle Edditing Mode* button (![](figures/tutorial/button-Qgis-AttributeTable-StartEditingMode.png ){width=4%}).
(@) Add 3 extra aquifers / features with the *Add feature* button (![](figures/tutorial/button-Qgis-AttributeTable-AddFeature.png){width=4%}).  
...
You probably tried to close the sheetpile box but either your lines cross or do not close actually when zooming in. To close the box, let's snap both start and end point together. 
...
Perhaps you are not satisfied with your created sheetpile pit. 

(@) You can play around with other nice options in QGIS to move (![](figures/tutorial/button_MoveFeature.png){width=4%}), rotate (![](figures/tutorial/button_RotateFeature.png){width=4%}) or scale (![](figures/tutorial/button_ScaleFeature.png){width=4%}) your box. 
...
(@) If you dare... try to rerun the model yourself with aquitard_c = 800 (layer 1) and see that the drawdawn is limited to the insite of the sheet pile box (@fig-figure_DrawdownSheetpileBox.)
(@) Don't forget to undo this side path and make aquitard_c = 0.5 again.

![Drawdown within the sheet pile box](figures/tutorial/figure_DrawdownSheetpileBox.png){width=60% #fig-figure_DrawdownSheetpileBox}

![](figures/tutorial/emoji-exclamation.png){width=5%} **How to reopen TIM after closing QGIS.** <br>
...
- your geopackage, the GPKG file, contains the complete set of model features and parameter values. Every time you change a model element, you remember you had to save the changes. So the GPKG file is always up to date. 
- your QGIS project is saved in the QGZ file and contains al the added layers, including your PGKG file, and their format (legend, line color, labels etc.).
...
The TIM panel is empty. *Why?* <br>
It is not possible for the plugin to read the Geopackage in the list. To restart you TIM project you must open the Geopackage from the TIM panel. This will add the geopackage to the Layers panel on the left. The following steps are the right way to go: 
...
retourbemaling (timml Line Sink Ditch:retour)

***Model 6: add observation points*** <br>

observatie (timml Observation:peilbuizen)
