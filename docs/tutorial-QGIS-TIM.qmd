...
- Download the tutorial material. [Follow this link.](https://deltares.thegood.cloud/s/oJE7qAHG3nZw2fk)
...
> 
> Perhaps your QGIS was installed in another language than English. While the Tutorial refers to the English version, let’s change to English. 
> 
...
> In case your work is mostly in The Netherlands and in the “Amersfoort / RD New” projection, consider to make this your default projection.
> 
...
(@) Click the Browse button (![](figures/tutorial/button-Qgis-browse.png){width=4%}) and from the tutorial dataset select the ZIP file "QGIS-Tim_training\\QGIS-iMOD-plugin.zip". 
...
(@) PDOK will find "Rijsenhout, Haarlemmermeer, Noord-Holland". Click on it and QGIS wil fly to the project location.
...
(@) User the browse button (![](figures/tutorial/button-Qgis-browse.png){width=4%}) and from the tutorial material select “…\\QGIS-Tim_training\\dbase\\bombs.shp”.
...
(@) Click your right mouse button and from the menu select *Attribute Table* to open the table in a new window. <br> **NB** Alternative is to press F6 or use the button *Open Attribute Tabll* (![](figures/tutorial/button-Qgis-OpenAttributeTable.png){width=3%}).
...
| aquifer_k   | 5     | [m/d]|
| aquifer_top | -10   | [m MSL] |
| aquifer_bottom | -30   | [m MSL]|
...
![](figures/tutorial/figure_WellFeatureAttributes.png){width=60%}

(@) Click *OK*.
(@) Click the *Toggle Editing* button (![](figures/tutorial/button-Qgis-AttributeTable-StartEditingMode.png){width=4%}) and if you are asked to save changes select *Save*. <br> An alternative way to save your edits is to click the *Save Layer Edits* button (![](figures/tutorial/button_SavelayerEdits.png){width=4%} and then stop editing with the *Toggle Editing* button (![](figures/tutorial/button-Qgis-AttributeTable-StartEditingMode.png){width=4%}).
...
(@) Unhide the output layer voor raster, probably named "case-Rijsenhout-output:raster".
...
**NB** You don't need to 'add' all four layers separately. Just use the mesh file "case-Rijsenhout-head_layer_0" and from the dropdown menu *Variable:* select the four bands/layers. 
...
(@) In the editing mode, the *Add Line Feature* button (![](figures/tutorial/button_AddLineFeature.png){width=4%}) is available. Click the button and draw a box arround the 5 bombs (click left to start, click right to close). 
...
(@) End the *Toggle Edditing Mode* with ![](figures/tutorial/button-Qgis-AttributeTable-StartEditingMode.png ){width=4%} and save your changes.
...
(@) If you dare... try to rerun the model yourself with aquitard_c = 800 (layer 1) and see that the drawdown is limited to the construction pit (@fig-figure_DrawdownConstructionPit.)
...
It is not possible for the plugin to read your geopackage as it is loaded in the layer list. To load your TIM project you *must* open the geopackage from the TIM panel. This will add the geopackage to the Layers panel next to the existing one with the same name, which makes it at least confusing. We better remove the original Geopackage first before opening it with TIM. Disadvantage is that we also remove the calculated results including all your formatting efforts. The developers try to fix that in the next version. <br> 
...
(@) Start editting with the *Toggle Edditing Mode* and use *Add Point Feature* button (![](figures/tutorial/button_AddPointFeature.png){width=4%}) to add 5 points in line with 1 really close to the dewatering well (see @fig-figure_ObservationsLocations).
...
***Model 7: determine the influence of the nearby lake*** <br>
...
The new results are added as new layer to the project. Check for instance sub-group "raster" under the group "case-Rijsenhout output". Now we use QGIS to calculate the head difference for layer 0.
...
(@) See that the difference in head is max 3 meter. 
(@) In *Geopackage* check again the Lake element.
(@) In *Compute* reset the path to the old name.

***Model 8: Abstraction effect over time*** <br>
Now you are ready for the last step: make your model transient. We keep it simpel: we run the model for 30 days and see the increasing effect of the well. 

(@) In *Compute* within the *output* section change "steady state" into "transient".
(@) Go to *Geopackage* and see that 5 elements have a transient component. 

By selecting the "transient" option, columns containing extra parameters necessary for a transient model have been unhide now. First of all we see it in the layer "timml Aquifer:Aquifer" where columns "storage" and "porosity are visible now. 

(@) In your geopackage select the layer "timml Aquifer:Aquifer".
(@) Open its table, start the editing mode and for every layer set both acquifer_s and acquitard_s to 0.001. Acquitard_s (layer 0) = 0.25.  

**NB!** In transient mode, TIM can not handle aquitard with zero thickness so lets's set the aquitard thickness to 0.1 m
...
(@) In your geopackage select the layer "ttim Computation Times:Domain" in order to definte the moments in time for which raster output is saved.   
(@) Open its table, start the editing mode and add 7 features / periods (click 6 times "Add feature").
...
(@) Open its table, start the editing mode and add 10 features / periods (click 6 times "Add feature").
...
- Simple: a well can be switched on and off only once. Parameters are set in "timml Well:DewateringWell".
- Detailed: a well can be switched on and off multiple times within the modeled period. Parameters are set in "ttim Well:DewateringWell".

In this training we show you the simple version. 

(@) In your geopackage select the layer "ttim Well:DewateringWell".   
(@) Open its table, start the editing mode and see that columns with time dependent parameters are visible now. 
(@) Switch off the Steady State abstraction: discharge = 0.
(@) Switch on the Transient abstraction: time_start=2, time_end=30, discharge_transient=2000, caisson_radius=1.
(@) Comput your transient model in the tab *Compute*. 

In case of a successful calculation a new layer is added to your Vector output: case-Rijsenhout-ttim Observation:Piezometers. This layer contains transient data which is indicted with the clock icon right from the layer name. There are 2  ways to visualize these calculated heads in the observations point a) animation over time and b) timeseries at location.
...
Only the first few days you see most prominent drawdown. Let's now display the timeseries at the point of your mouse. 

(@) In the iMOD time series panel be sure the layer "case-Rijsenhout-head_layer_0" is selected. 
...
(@) Click the *Plot* button and the selected timeseries are added to the chart. Colors can be changed.  
...
***Export your TIM model to Python*** <br>
...
(@) Check the content of the file with yout texteditor (e.g. Notepad).
